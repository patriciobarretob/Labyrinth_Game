...
public class Board {
  ...
  public boolean validSlideRow(int row) {
    return validSlideIndex(row);
  }

  public boolean validSlideCol(int col) {
    return validSlideIndex(col);
  }

  //TODO: get rid of % 2 and handle valid row and column indices
  private boolean validSlideIndex(int index)  {
    if (!validIndex(index)) {
      return false;
    }
    if (index % 2 == 1) {
  ...
  public List<Coordinate> getReachablePositions(Coordinate location) {
    if (!validIndex(location.x) || !validIndex(location.y)) {
  ...
  public boolean isPositionReachable(Coordinate start, Coordinate target) {
    if (!validIndex(start.x) || !validIndex(start.y)
    || !validIndex(target.x) || !validIndex(target.y)) {
      throw new IllegalArgumentException("Not a valid index");
    }
  ...
  private static class RowColumnOrder implements Comparator<Coordinate> {
  ...
  public boolean isInBounds(Coordinate location) {
    return validIndex(location.x) && validIndex(location.y);
  }
